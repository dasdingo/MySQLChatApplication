	...
	public List<ConversationReply> getAllMessages(String dBUserReceiverString) {
		...
		if (dBUserReceiver == null) {
			...
			if (conversationService.findByUsers(dBUserSender, dBUserReceiver) != null) {
				conversationReplyList = conversationReplyService
						.findByConversationID(conversationService.findByUsers(dBUserSender, dBUserReceiver).get_id());
				if (conversationReplyList != null) {
					for (ConversationReply conversationRep : conversationReplyList) {
						localCount++;
						System.out
								.println(dBUserService.findById(conversationRep.get_user_id_fk().getId()).getUserName()
										+ ": " + conversationRep.get_reply());
					}
